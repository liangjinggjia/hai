...
<script type="text/javascript">
    var rejectDataGrid;
    $(function() {
        rejectDataGrid = $('#rejectDataGrid').datagrid({
            url : '${path }/check/getreject',
            striped : true,
            rownumbers : true,
            pagination : true,
            singleSelect : true,
                ...
                formatter : function(value, row, index) {
                    var str = '';
                        <shiro:hasPermission name="/check/modify">
                            str += $.formatString('<a href="javascript:void(0)" class="resource-easyui-linkbutton-edit" data-options="plain:true,iconCls:\'fi-align-justify icon-black\'" onclick="rejectdetailFun(\'{0}\');" >详细</a>',row.postId);
                        </shiro:hasPermission>
...
//用于搜索栏
    function searchRejectFun() {
        $("#rejectDataGrid").empty();
        rejectDataGrid.datagrid('load', $.serializeObject($('#searchUserForm')));
    }
    function cleanRejectFun() {
        $('#searchUserForm input').val('');
        userDataGrid.datagrid('load', {});
    }
    function rejectdetailFun(id) {
        if (id != undefined) {
            var opts = {
                title : id+'详细',
                border : false,
...
//                iconCls : node.iconCls
            ...
            var url = "/check/postrejectdetail/"+id;
...
<div class="easyui-layout" data-options="fit:true,border:false">
            ...
            <table>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-magnifying-glass',plain:true" onclick="searchRejectFun();">查询</a>
                        <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-x-circle',plain:true" onclick="cleanRejectFun();">清空</a>
                    </td>
                </tr>
            </table>
        </form>
    ...
    <div data-options="region:'center',border:false"  style="overflow: hidden;">
        <table id="rejectDataGrid"></table>
    </div>
</div>
<div id="modifyToolbar" style="display: none;">
    <shiro:hasPermission name="/check/modify">
        <a onclick="" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'fi-plus icon-green'">添加</a>
    </shiro:hasPermission>
</div>