...
$(function(){
	function e(){
		...
		//parseInt是将字符串解析成int整数
		// b,d=0由d决定哪个li出现消失
		$(".arrowdown").click(function(){
			a&&parseInt($(".list li:first").css("marginTop"))> -110*h&&
			//.eq(index)index整数，指示元素的位置（最小为 0）。如果是负数，则从集合中的最后一个元素往回计数。
			// (d++,b--,a=!1,$(".list li").eq(d-1).liOut(),$(".list li").eq(d+row-1).liIn(),$(".list li:first")
			(d++,b--,a=!1,$(".list li").eq(h+d-1).liOut(),$(".list li").eq(h+d+row-1).liIn(),$(".list li:first")
				// animate() 方法执行 CSS 属性集的自定义动画。$(selector).animate(styles,speed,easing,callback)
				// .animate({marginTop:110*b},600,"easeInOutQuad",function(){
				.animate({marginTop:110*(b-h)},600,"easeInOutQuad",function(){
				a=!0;$(".arrowdown").css("opacity",1);
				$(".arrowdown").removeClass("arrow_active")
			}),e()
		)});
		$(".arrowup").click(function(){
			a&&0!=parseInt($(".list li:first").css("marginTop"))&&
			// (b++,d--,a=!1,$(".list li").eq(d).liIn(),$(".list li").eq(d+row).liOut(),$(".list li:first")
			(b++,d--,a=!1,$(".list li").eq(h+d).liIn(),$(".list li").eq(h+d+row).liOut(),$(".list li:first")
				// .animate({marginTop:110*b},600,"easeInOutQuad",function(){
				.animate({marginTop:110*(b-h)},600,"easeInOutQuad",function(){
		...
		// 见70行var b=0,f=$(".list li").length,d=0,a=!0,c;
        $(".list li:lt("+(f-3)+")").find(".lileft").css({left:"-400px"});
		$(".list li:lt("+(f-3)+")").find(".liright").css({right:"-700px"});
